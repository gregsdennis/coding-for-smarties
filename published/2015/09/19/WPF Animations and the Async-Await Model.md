...
The button is clicked, calling the handler method _Button_Clicked().
The first animation begins.
...
Since the method has ended, the Dispatcher is now free to select the next task. It does this by priority, and since the animation has an ultra-low priority, the async call wins.
The async call completes, but there is a continuation on the task. This is placed in the queue.
The Dispatcher makes another selection, which is the continuation, further deferring the animation.
The second animation begins, placing a management task in the queue, ending the continuation task.
The Dispatcher is now free again to make another selection, and it can handle both animations normally.
