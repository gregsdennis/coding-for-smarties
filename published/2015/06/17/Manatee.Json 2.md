...
JSON as a Language
JSON.org has a great review of the language, including some fantastic flow charts illustrating the syntaxes of each part. Here’s a brief summary.
...
You are in a state.
You receive an input.
Processing that input should put you in another state.
Repeat 2 & 3 until there are no more inputs (or until an ‘end’ input is found).
...
start	goto value			
value	add value
goto delimiter	add value
goto delimiter		end (empty array only)
delimiter			goto value	end
For objects:

OBJECT	object {	array [ / number /
boolean / null	string	,	:	}
start	goto key					
key			store key
goto colon			end (empty object only)
colon					goto value	
value	add key/value
goto delimiter	add key/value
goto delimiter	add key/value
goto delimiter			
delimiter				goto key		end
To implement these tables, we use the same approach as with my math expression assignment from school. For Manatee.Json, I ended up putting together a generic state machine, Manatee.StateMachine. I’ll leave the details of how that works for another post. For now, a brief overview will suffice.
...
"	
Scan the input string for the next " (but not \").
Extract that substring.
Process the substring for delimited characters.
[	Use JsonArray to parse.
{	Use JsonObject to parse.
Anything else	
Scan the input string for the next ,, ], or }.
Extract that substring.
If it’s true, false, or null, use the appropriate type, otherwise
Assume it’s a number and use Double.TryParse(). If that fails throw a JsonValueParseException (we don’t recognize it).
...
And that’s a general overview of parsing. Feel free to download the source to have a closer look.
